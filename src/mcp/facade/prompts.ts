  ...
  "kotikit.create_figma_draft": {
    ...
    render: (args) =>
      [
        ...
        "Read the apply packet evidence checklist, use planned existing components as the actual visible UI, then scan the applied root node.",
        "Record transactionId, node id, Figma node type, bounds, component refs or componentKey, component source, variable refs, required icon refs, autoLayout, and evidenceSnapshot with kotikit_record_figma_apply.",
        "Never add hidden, low-opacity, or off-canvas proof nodes; repair the visible frame if evidence fails.",
