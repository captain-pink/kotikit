...
- Apply Figma drafts incrementally through official Figma MCP. Fetch the graph apply packet with kotikit_get_artifact, apply only the active Figma transaction with use_figma, record transactionId, node id, bounds, component refs, component source, variable refs, required icon refs, and auto-layout metadata with kotikit_record_figma_apply, then continue the run. Do not create every screen state in one opaque Figma write.
