...
describe("verifyGateEnvironment", () => {
  it("returns ok when all tools are present (vitest)", async () => {
    ...
    const report = await verifyGateEnvironment({ root, adapter: reactAdapter, testFramework: "vitest" });
  ...
  it("returns ok when tools present and testFramework=none", async () => {
    ...
    const report = await verifyGateEnvironment({ root, adapter: reactAdapter, testFramework: "none" });
    expect(report.ok).toBe(true);
  });
  ...
  it("lists all four missing gates with install hints (vitest)", async () => {
    ...
    expect(report.missing.map(m => m.gate).sort()).toEqual(["eslint", "prettier", "tsc", "vitest"]);
  ...
  it("works with a stub adapter", async () => {
    const stub: Adapter = {
      ...
      testScaffold: () => "",
      qualityGates: () => [],
      verifyEnvironment: async () => ({ ok: false, missing: ["tsc", "eslint"] as const }) as { ok: false; missing: ("tsc" | "eslint" | "prettier" | "vitest")[] },
      transformGateOutput: () => ({ failures: [] }),
    };
    const report = await verifyGateEnvironment({ root: "/tmp/x", adapter: stub, testFramework: "vitest" });
    expect(report.ok).toBe(false);
