...
- `FigmaClient#getComments(fileKey, { asMarkdown? })` — read compact Figma
  comments for the lightweight `review-screen` feedback loop
...
**Normalization handles different published-library shapes.** Published Figma libraries do not all look the same through the API. Some publish clean component-set metadata, while others return every variant child as a separate published component and only expose its logical set through `containing_frame.containingComponentSet`. `normalizePublishedDesignSystem` groups rows by `component_set_id`, `containingComponentSet.nodeId`, or `containingStateGroup.nodeId`; without a set ID, each published key stays separate even when set names match. Component-set metadata is indexed by both published key and node ID. When Figma omits variant definitions for an identified set, the normalizer fetches its node details; if those are still missing, it infers variant axes from child names like `Size=md, Type=Fill`.
...
Figma token resolution is used by sync. Kotikit loads the target project's
`.env` before constructing a Figma client. If `config.figma.token` is omitted
or empty, it resolves `${FIGMA_TOKEN}` by default. If `config.figma.token` is
present, that explicit value wins and may be a plain token, `${OTHER_ENV_VAR}`,
or an `op://` secret reference. `.env` loading does not replace non-empty shell
values, but it does refresh empty placeholders such as the scaffolded
