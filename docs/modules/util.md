...
`findProjectRoot` walks the directory tree upward until it finds a directory containing `.kotikit/`. This allows kotikit to work correctly when Claude Code opens a file inside a subdirectory of the project root. If no `.kotikit/` ancestor is found (e.g. in a fresh project that has not yet been initialized), it returns the starting directory, which is safe because `configExists` will return `false` and the init flow will trigger.
